...
class EquipmentRepository {
  ...
  CollectionReference<Map<String, dynamic>> get _equipmentItemsRef =>
      _firestore.collection('equipmentItems');
  CollectionReference<Map<String, dynamic>> get _equipmentCategoriesRef =>
      _firestore.collection('equipmentCategories');
  CollectionReference<Map<String, dynamic>> get _equipmentPiecesRef =>
      _firestore.collection('equipmentPieces');
  CollectionReference<Map<String, dynamic>> get _equipmentLogsRef =>
      _firestore.collection('equipmentLogs');
  CollectionReference<Map<String, dynamic>> get _employeesRef =>
      _firestore.collection('employees');

  DocumentReference<Map<String, dynamic>> get _counterRef =>
      _firestore.collection('counter').doc('count');

  Future<List<EquipmentItem>> getEquipmentItems() async {
    final querySnapshot = await _equipmentItemsRef.get();
    return querySnapshot.docs
        .map((doc) => EquipmentItemMapper.fromMap(doc.data()))
        .toList();
  }
  ...
  Future<List<EquipmentLog>> getEquipmentLogs() async {
    ...
    return querySnapshot.docs
        .map((doc) => EquipmentLogMapper.fromMap(doc.data()))
        .toList();
  }
  ...
  Future<List<EquipmentCategory>> getCategories() async {
    ...
    return querySnapshot.docs
        .map((doc) => EquipmentCategoryMapper.fromMap(doc.data()))
        .toList();
  }
  ...
  void updateEquipmentPieces(
    List<EquipmentPiece> pieces,
    String? renterID,
    Timestamp? lastRented,
  ) {
  ...
  Future<List<EquipmentPiece>> fetchEquipmentPieces({
    ...
    return querySnapshot.docs
        .map((doc) => EquipmentPieceMapper.fromMap(doc.data()))
        .toList();
  }
  ...
  Future<EquipmentLog> completeSubmission(EquipmentLog log) async {
    final logWithSubmissionDetails = log.copyWith(
      collectedTime: Timestamp.now(),
      collectorID: currentEmployee?.id,
    );
    await _equipmentLogsRef
        .doc(logWithSubmissionDetails.id)
        .set(logWithSubmissionDetails.toMap());
  ...
  Future<List<EquipmentPiece>> getEquipmentPieces(String id) async {
    final querySnapshot =
        await _equipmentPiecesRef.where('equipmentItemID', isEqualTo: id).get();
    return querySnapshot.docs
        .map((doc) => EquipmentPieceMapper.fromMap(doc.data()))
        .toList();
  }
  ...
  Future<void> deleteEquipmentItem(EquipmentItem equipmentItem) async {
    ...
    final querySnapshot = await _equipmentPiecesRef
        .where('equipmentItemID', isEqualTo: equipmentItem.id)
        .get();
